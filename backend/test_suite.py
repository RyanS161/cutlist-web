...
def _classify_part(sorted_dims: List[float]) -> Dict[str, Any]:
    ...
    Matches against PART_LIBRARY which is a flat dict:
      - Beams: {'x': 28, 'y': 28, 'z': 200}
      - Plywood: {'z': 7, 'max_width': 500, 'max_height': 500}
    """
    tolerance = 1.0

    # Try to find an exact beam match in the library
    # Beams have 'x', 'y', 'z' keys (all fixed dimensions)
    best_beam_match = None
    best_beam_name = None
    best_beam_diff = float('inf')

    for part_name, specs in PART_LIBRARY.items():
        if 'x' in specs and 'y' in specs and 'z' in specs:
            # It's a beam — compare sorted dims
            lib_dims = sorted([specs['x'], specs['y'], specs['z']])
            diff = (
                abs(sorted_dims[0] - lib_dims[0])
                + abs(sorted_dims[1] - lib_dims[1])
                + abs(sorted_dims[2] - lib_dims[2])
            )
            if diff < best_beam_diff:
                best_beam_diff = diff
                best_beam_match = specs
                best_beam_name = part_name

    if best_beam_match is not None and best_beam_diff <= tolerance * 3:
        return {
            'type': best_beam_name,
            'cross_section': (best_beam_match['x'], best_beam_match['y']),
            'length': best_beam_match['z'],
            'valid': True,
            'match_diff': round(best_beam_diff, 2),
        }

    # Check plywood entries (have 'z' + 'max_width' + 'max_height')
    for part_name, specs in PART_LIBRARY.items():
        if 'max_width' in specs and 'max_height' in specs:
            thickness = specs['z']
            if abs(sorted_dims[0] - thickness) <= tolerance:
                valid_size = (
                    sorted_dims[1] <= specs['max_width'] + tolerance
                    and sorted_dims[2] <= specs['max_height'] + tolerance
                )
                return {
                    'type': part_name,
    ...
    for i, part_data in enumerate(parts):
        ...
        if classification['type'] == 'unknown':
            violations.append(
                ...
                f"— does not match any entry in the parts library"
            )
        elif 'valid_size' in classification and not classification['valid_size']:
            # Plywood with dimensions exceeding max
            violations.append(
                f"Part '{name}': Plywood size {classification['width']:.1f}x{classification['height']:.1f}mm "
                f"exceeds maximum allowed dimensions"
            )
